...
private let strings: [String: (String, String)] = [
    ...
    "power.hint":          ("「被窗口遮挡时暂停」建议一直开着 —— 你开全屏 app 的时候壁纸根本看不见，继续解码纯粹是白烧电、白发热。",
                            "Keep \"pause when covered\" on — when a fullscreen app hides the wallpaper, decoding it anyway just burns battery and heat for nothing."),
    ...
    "playlist.enable":     ("使用播放列表", "Use playlist"),
    "playlist.section":    ("视频列表", "Videos"),
    "playlist.add":        ("添加视频…", "Add Videos…"),
    "playlist.remove":     ("移除选中", "Remove Selected"),
    "playlist.clear":      ("清空", "Clear"),
    "playlist.empty":      ("列表是空的，先添加视频", "The list is empty — add some videos"),
    ...
    "playlist.overridden": ("日程功能开着，它的优先级更高 —— 列表现在不生效。",
                            "The schedule is on and takes priority — the playlist is inactive right now."),
    ...
    "schedule.hint":       ("开着的时候日程优先于播放列表和单视频。没设视频的那个时段会沿用另一个。",
                            "While on, the schedule overrides the playlist and the single video. A period with no video falls back to the other one."),
